...
private func delay<Value>(@autoclosure(escaping) body: Void throws -> Value, by interval: NSTimeInterval, @autoclosure(escaping) onCancel produceError: () -> ErrorType) -> Task<Value> {
    typealias Result = TaskResult<Value>

    let deferred = Deferred<Result>()
    let queue = Deferred<Value>.genericQueue

    defer {
        afterDelay(interval, upon: queue) {
...
class TaskTests: XCTestCase {
    ...
    func testThatFlatMapForwardsCancellationToSubsequentTask() {
        let beforeTask = delay(1, by: 0.25, onCancel: Error.Third)
    ...
    func testThatThrowingFlatMapSubstitutesWithError() {
        let beforeTask: Task<Int> = delay(1, by: 0.25, onCancel: Error.Third)
...
class TaskCustomExecutorTests: CustomExecutorTestCase {
    ...
    func testThatFlatMapForwardsCancellationToSubsequentTask() {
        ...
        let afterTask: Task<String> = beforeTask.flatMap(upon: executor) { _ in
            Task(Future(), cancellation: afterExpectation.fulfill)
        }
